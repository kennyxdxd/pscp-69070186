...
```text
input
จำนวนร้านเเละจำนวนช่วงเวลาที่ต้องตรวจสอบ
เวลาเปิด เวลาปิด
จำนวนเวลาที่ต้องตรวจสอบ
output 
จำนวนร้านที่เปิด ในเวลาที่ตรวจสอบ

โดยโจทย์ต้องการให้เราตรวจดูว่าในช่วงเวลาที่ตรวจสอบมีร้านเปิดอยุ่กี่ร้าน
```
...
```text
step 1 รับค่ามา
step2 loop เเละ รับค่าเวลาเปิดปิดมา เเล้ว append ใส list โดย append เป็น [open,close]
step3 รับค่าเวลาที่เราต้องตรวจสอบ
step4 Loop ช่วงเวลาที่ต้องตรวจสอบ เเล้วก็ loop list ใน step 2โดยตั้งเงื่อนใขว่า ถ้า ช่วงเวลา open กับ close อยู่ในช่วงเวลาที่เราตรวจสอบ ค่า count ก็จะเพิ่ม จากนั้นก็ append count ลงใน list 
step 5 print(" ".join(answer))โดยที่ answer จะเป็นlist ที่เราเก็บค่า count อยู่
```
...
```text
วิธีสุดท้ายของฉันเหมือนกับแผนแรก โดยต้องรับค่า open กับ close จะ append เป็น list อีกที [open,close]
```
...
```text
มี case ที่เวลาตรวจตรงกับเวลาปิดพอดี
```
